...
FOR i=0L,nplanfiles-1 do begin
  ...
  For j=0L,nframes-1 do begin
    ...
    if keyword_set(clobber) or $
      ...
      ;----------------------------------
      print,'STEP 4: Telluric Correction with APTELLURIC_HACK'
      if planstr.platetype eq 'single' then begin
      ...
      APTELLURIC_HACK,frame_skysub,plugmap,frame_telluric,tellstar,starfit=2,$
      ...
      if n_elements(telerror) gt 0 and planstr.platetype ne 'single' and not keyword_set(force) then begin
        print,'not halted: APTELLURIC_HACK Error: ',telerror
        ntellerror+=1
  ...
  if ntellerror gt 0 then begin
    print, ntellerror,' frames had APTELLURIC_HACK errors'
    if keyword_set(halt) then stop,'halt: ', ntellerror,' frames had APTELLURIC_HACK errors'
  endif
