...
;  shift=shift_rnd
  ...
  ;;-------------------------
  for jfiber=0,nfibers-1 do begin
    if (jfiber+1) mod 50 eq 0 then print,strtrim(jfiber+1,2),'/',strtrim(nfibers,2)
    ...
    iplugind = where(plugmap.fiberdata.spectrographid eq 2 and $
                     plugmap.fiberdata.fiberid eq 300-jfiber,niplugind)
	...
	temperr = smooth(medfilt1d(data2.err,501,edge=2),100,/nan)
      ...
      shift_i0 = allframes[i0].shift.chipfit[0]*jfiber+allframes[i0].shift.chipfit[ichip+1] 
      shift_i1 = allframes[i1].shift.chipfit[0]*jfiber+allframes[i1].shift.chipfit[ichip+1] 
      shift_i2 = allframes[i2].shift.chipfit[0]*jfiber+allframes[i2].shift.chipfit[ichip+1] 
      ...
      for ibit=0,n_elements(flag)-1 do begin
        ...
        if nmask1 gt 0 or nmask2 gt 0 then begin
          ...
          if nbd gt 0 then combframe.(ichip).mask[bd,jfiber]=combframe.(ichip).mask[bd,jfiber] OR vmask
        endif
      endfor
      ;; If this maskval corresponds to a bad pixel, inflate the error
      bd = where(combframe.(ichip).mask[*,jfiber] and badmask(),nbd)
      if nbd gt 0 then begin
        combframe.(ichip).err[bd,jfiber] *= 10.
      ;  combframe.(ichip).flux[bd,jfiber]=!values.f_nan
      endif
      ...
      if Nwavelength gt 0 then begin
        w1 = data1.wavelength
        w2 = data2.wavelength
        wcoef1 = reform(frame1.(ichip).wcoef[jfiber,*])
        wcoef2 = reform(frame2.(ichip).wcoef[jfiber,*])
        ...
        wave1 = dblarr(npix*2)
        wave1[0:npix*2-2:2] = wave1_lfthalf
        wave1[1:npix*2-1:2] = wave1_rthalf

        wave2_lfthalf = PIX2WAVE(y+0.0-abs_shift-shift,wcoef2)
        wave2_rthalf = PIX2WAVE(y+0.5-abs_shift-shift,wcoef2)
        wave2 = dblarr(npix*2)
        wave2[0:npix*2-2:2] = wave2_lfthalf
        wave2[1:npix*2-1:2] = wave2_rthalf
        ...
        if keyword_set(avgwave) then begin
          ...
          newy = dblarr(npix*2)
          newy[0:npix*2-2:2] = y+0.0-abs_shift
          newy[1:npix*2-1:2] = y+0.5-abs_shift
        ...
        ;; Use the frame1 wavelengths (old way)
          ...
          combwave[0:npix*2-2:2] = wave1_lfthalf
          combwave[1:npix*2-1:2] = wave1_rthalf
          combwcoef = wcoef1
    ...
    if keyword_set(pl) then begin
      ...
      yy = [combframe.(0).flux[*,jfiber], combframe.(1).flux[*,jfiber], combframe.(2).flux[*,jfiber] ]
      ...
      for k=0,2 do oplot,findgen(npix*2)+npix*2*k+k*300,combframe.(k).flux[*,jfiber]
      ;oplot,combspec
  ...
  if nlsfcoef gt 0 then if combframe.(0).lsfcoef[0,0] eq 1 then begin
    ...
    for ichip=0,2 do begin
      ...
      for jfiber=0,nfibers-1 do begin
        ...
        lsfpar = reform(combframe.(ichip).lsfcoef[jfiber,*])
...
;                  data.mask and badmask(),nbdpix)  ; 5 sigma outliers
  ...
  if keyword_set(pl) then begin
    ...
    yy = [outframe.(0).flux[*,jfiber], outframe.(1).flux[*,jfiber], outframe.(2).flux[*,jfiber] ]
    ...
    plot,[0],[0],/nodata,xr=xr,yr=yr,xs=1,ys=1,xtit='Pixel',ytit='Counts',$
         tit=strtrim(fiberobjid,2)+' (OBJTYPE='+fiberobjtype+')'
    for k=0,2 do oplot,findgen(npix2)+npix2*k+k*300,outframe.(k).flux[*,jfiber]
    ;oplot,combspec
    ;oplot,sqrt(combvar),co=250
    ;oplot,xx,yy
