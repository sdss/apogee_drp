...
;stop
  ...
  if iter eq niter-1 and keyword_set(pltelstarfit) and $
    ...
    !p.multi=[0,1,2]
    plot,outstr.x,outstr.spec,/nodata,xtit='Pixels',ytit='Counts',xs=1,ys=1,xr=xr,yr=yr,tit='Fiber '+strtrim(ifiber,2),co=0,yticklen=0.1
    ...
    plot,outstr.x,outstr.spec,/nodata,xtit='Pixels',ytit='Counts',xs=1,ys=1,xr=[3000,5000],yr=yr,co=0,yticklen=0.1
    oplot,outstr.x,outstr.spec,co=70
    ;oplot,x[gd],yfit1,co=250,linestyle=2
    ;oplot,x,yfit1*smspec,co=250,linestyle=2
    oplot,outstr.x,outstr.telluric*outstr.cont,co=250,linestyle=2
...
for i=0,nfibers-1 do begin
  ...
  if keyword_set(debugpl) then begin
    ...
    legend_old,['Original','Corrected'],textcolor=[255,250],/bottom,/left
    print,normpar
  endif
