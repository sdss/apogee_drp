...
cols = 'x.catalogid,' + strjoin('t.'+colarr,',')
if n_elements(catalogid) gt 1 then begin
  ids = strjoin(strtrim(catalogid,2),',')
  sql = "select "+cols+" from catalogdb.tic_v8 as t join catalogdb.catalog_to_tic_v8 as x on x.target_id=t.id where x.catalogid in ("+ids+")"
endif else begin
  ids = strtrim(catalogid[0],2)
  sql = "select "+cols+" from catalogdb.tic_v8 as t join catalogdb.catalog_to_tic_v8 as x on x.target_id=t.id where x.catalogid="+ids
endelse
