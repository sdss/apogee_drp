...
def makesumfile2(telescope='lco25m',apred='daily', ndo=None):
    ...
    for i in range(100):
        ...
        tdifmin = tdif[g][0]
        if tdifmin < 1/24:
            print('  adding Magellan-Baade seeing data ('+str("%.3f" % round(magMJD1[g][0],3))+'-'+str("%.3f" % round(t.mjd,3))+' = '+str("%.3f" % round(tdifmin*24*60,3))+' minutes)')
            outstr['SEEING_BAADE'][i] = magSeeing1[g][0]
        tdif = np.abs(t.mjd - magMJD2)
        g, = np.where(tdif == np.nanmin(tdif))
        tdifmin = tdif[g][0]
        if tdifmin < 1/24:
            print('  adding Magellan-Clay seeing data ('+str("%.3f" % round(magMJD2[g][0],3))+'-'+str("%.3f" % round(t.mjd,3))+' = '+str("%.3f" % round(tdifmin*24*60,3))+' minutes)')
