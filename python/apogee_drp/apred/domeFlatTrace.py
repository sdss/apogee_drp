...
###################################################################################################
def findBestFlatExposure(apred='daily', telescope='apo25m', medianrad=100, expnum=36760022, silent=False):
    start_time = time.time()
...
def findBestFlatSequence(plate='15000', mjd='59146', telescope='apo25m', apred='daily'):
    ...
    n_ims = len(ims)
    for i in range(n_ims):
        psfid = findBestFlatExposure(apred=apred, telescope=telescope, expnum=ims[i], silent=True)
        print('sci exposure ' + str(ims[i]) + '----> dflat ' + str(psfid))
