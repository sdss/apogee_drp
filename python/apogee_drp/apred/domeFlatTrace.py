...
def findBestFlatExposure(domeTable=None, refpix=None, twodfiles=None, medianrad=100, minflux=None, highfluxfrac=None, silent=True):
    ...
    for ichip in range(nchips):
        ...
        medFWHM = np.nanmean(gpeaks['pars'][:, 2])*2.354
        medDomeFWHM = np.nanmean(domeTable['GAUSS_SIGMA'][:, ichip, gpeaks['num']], axis=1)*2.354
        gd, = np.where(np.absolute(medFWHM - medDomeFWHM) < 0.1)
