    ...
    if makeplots is True:
        ...
        if (os.path.exists(plotfile) == False) | (clobber == True):
            ...
            ax.legend(loc='upper left', ncol=2, columnspacing=0.5, labelspacing=1.3, handletextpad=0.3, markerscale=1, 
                      fontsize=fontsize, edgecolor='k', borderaxespad=2, framealpha=1)
        ...
        if os.path.exists(qtracefile):
            ...
            if (os.path.exists(plotfile) == False) | (clobber == True):
                ...
                for ichip in range(nchips):
                    ...
                    for axis in ['top','bottom','left','right']: ax.spines[axis].set_linewidth(axthick)
                    if ichip == nchips-1: ax.set_xlabel(r'JD - 2,400,000')
                    if ichip == 1: ax.text(-0.06, 0.5, r'Quartz Trace Residuals (pix)', transform=ax.transAxes, ha='right', va='center', rotation=90)
                    if ichip < nchips-1: ax.axes.xaxis.set_ticklabels([])
                    ax.axhline(y=0, color='k', linestyle='dashed', alpha=alf)
                    ax.axvline(x=59146, color='teal', linewidth=2)
                    ax.axvline(x=startFPS, color='teal', linewidth=2)
        ...
        if os.path.exists(qtracefile):
            ...
            if (os.path.exists(plotfile) == False) | (clobber == True):
                ...
                for ichip in range(nchips):
                    ...
                    for axis in ['top','bottom','left','right']: ax.spines[axis].set_linewidth(axthick)
                    if ichip == nchips-1: ax.set_xlabel(r'JD - 2,400,000')
                    if ichip == 1: ax.set_ylabel(r'FWHM (pixels)')
                    if ichip < nchips-1: ax.axes.xaxis.set_ticklabels([])
                    ax.axvline(x=59146, color='teal', linewidth=2)
                    ax.axvline(x=startFPS, color='teal', linewidth=2)
                    if ichip == 0: ax.text(59146-xspan*0.005, ymax-yspan*0.04, 'plate-III+IV', fontsize=fontsize, color='teal', va='top', ha='right', bbox=bboxpar)
