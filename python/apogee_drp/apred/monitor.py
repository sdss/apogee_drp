    ...
    if makecomplots is True:
        ...
        if (os.path.exists(plotfile) == False) | (clobber == True):
            ...
            for ichip in range(nchips):
                ...
                if ichip == 1: ax.text(-0.05, 0.5, r'S/N per minute ($' + magmin + r'>=H>' + magmax + r'$)', transform=ax.transAxes, rotation=90, ha='right', va='center')
                ...
                sc1 = ax.scatter(xvals, yvals, marker='o', s=markersz, c=scolors, cmap='copper_r')#, c=colors[ifib], alpha=alf)#, label='Fiber ' + str(fibers[ifib]))
