...
#        if len(check) < 1: subprocess.call(['scp', pfiles[i], sdir5 + 'flatflux/'])
    ...
    if makecomplots is True:
        ...
        if (os.path.exists(plotfile) == False) | (clobber == True):
            ...
            if ichip == 1: ax.text(1.06, 0.5, r'MJD',ha='left', va='center', rotation=-90, transform=ax.transAxes)

            fig.subplots_adjust(left=0.06,right=0.95,bottom=0.06,top=0.96,hspace=0.08,wspace=0.00)
            plt.savefig(plotfile)
            plt.close('all')
