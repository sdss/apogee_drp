...
def monitor(instrument='apogee-n', apred='daily', clobber=True, makesumfiles=False):
    ...
    if (os.path.exists(plotfile) == False) | (clobber == True):
        ...
        for ichip in range(nchips):
            ...
            ax.scatter(tcaljd, flux[:, ichip, 0, 0] / tcal['NREAD']*10.0, marker='*', s=60, edgecolors='purple', color='white', alpha=alpha)#, label='Fiber 290')
            ax.scatter(tcaljd, flux[:, ichip, 2, 0] / tcal['NREAD']*10.0, marker='^', s=30, edgecolors='darkorange', color='white', alpha=alpha)#, label='Fiber 150')
            ax.scatter(tcaljd, flux[:, ichip, 4, 0]  / tcal['NREAD']*10.0, marker='o', s=30, edgecolors='green', color='white', alpha=alpha)#, label='Fiber 10')
