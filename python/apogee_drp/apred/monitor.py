...
def monitor(instrument='apogee-n', apred='daily', clobber=True, makesumfiles=False, allplots=True):
    ...
    if (os.path.exists(plotfile) == False) | (clobber == True):
        ...
        ax1.scatter(allsci['MOONPHASE'], allsci['SKY'][:, 1], marker='o', s=markersz, c=allsci['MOONDIST'], cmap='plasma_r', alpha=alf, vmin=0, vmax=90)
        gd, = np.where((allsci['ZERO'] != -np.inf) & (allsci['ZERO'] < 20) & (allsci['ZERO'] > 0))
        ax2.scatter(allsci['MOONPHASE'][gd], allsci['SKY'][gd][:, 1], marker='o', s=markersz, c=allsci['ZERO'][gd], cmap='plasma_r', alpha=alf, vmin=17, vmax=19)
