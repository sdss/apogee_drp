    ...
    if makeplots is True:
        ...
        if (os.path.exists(plotfile) == False) | (clobber == True):
            ...
            for ichip in range(nchips):
                ...
                if ichip == 1: ax.text(1.07, 0.5, r'Moon Phase',ha='left', va='center', rotation=-90, transform=ax.transAxes)

            fig.subplots_adjust(left=0.05,right=0.949,bottom=0.06,top=0.96,hspace=0.08,wspace=0.00)
            plt.savefig(plotfile)
            plt.close('all')
