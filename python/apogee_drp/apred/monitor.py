...
#        if len(check) < 1: subprocess.call(['scp', pfiles[i], sdir5 + 'flatflux/'])
    ...
    if makecomplots is True:
        ...
        if (os.path.exists(plotfile) == False) | (clobber == True):
            ...
            g, = np.where(pl < 10000)
            upl,uind = np.unique(meanh[g], return_index=True)
            ax.scatter(upl, meanh[g][uind], marker='o', s=50, c='cyan', edgecolors='k')#, c=colors[ifib], alpha=alf)#, label='Fiber ' + str(fibers[ifib]))
            g, = np.where(pl > 10000)
            ax.scatter(pl[g]-10000, meanh[g], marker='o', s=50, c='red', edgecolors='k')#, c=colors[ifib], alpha=alf)#, label='Fiber ' + str(fibers[ifib]))
