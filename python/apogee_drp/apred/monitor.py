    ...
    if makeplots is True:
        ...
        if ((os.path.exists(qplotfile) == False) | (clobber == True)) & (os.path.exists(qfile)):
            ...
            for irow in range(nrows):
                ...
                if irow == 0:
                    sc1 = ax.scatter(xvals, yvals, marker='o', s=markersz*1.2, c=cvals, cmap='rainbow', vmin=0.5, vmax=2.0)
                else:
                    sc1 = ax.scatter(xvals, yvals, marker='o', s=markersz*1.2, c=cvals, cmap='rainbow')
                ...
                xx = [np.min(xvals[plate]),np.max(xvals[plate])]
                yy = [np.nanmedian(yvals[plate]), np.nanmedian(yvals[plate])]
                pl1 = ax.plot(xx, yy, c='navy', linewidth=2, label='plate median ('+str(int(round(np.nanmedian(yvals[plate]))))+')')

                if len(fpsi) > 0: 
                    xx = [np.min(xvals[fpsi]),np.max(xvals[fpsi])]
                    yy = [np.nanmedian(yvals[fpsi]), np.nanmedian(yvals[fpsi])]
                    pl2 = ax.plot(xx, yy, c='magenta', linewidth=2, label='FPS median ('+str(int(round(np.nanmedian(yvals[fpsi]))))+')')
                    if irow == 0:
