...
def visitcomb(allvisit,starver,load=None, apred='r13',telescope='apo25m',nres=[5,4.25,3.5],bconly=False,
    ...
    if dorvfit:
        try:
            ...
            logger.error('Exception raised in visitcomb RV for: ', header.get['FIELD'],apstar.header['OBJID'])
            logger.error("ValueError: {0}".format(err))
        except RuntimeError as err:
            logger.error('Exception raised in visitcomb RV for: ', header.get['FIELD'],apstar.header['OBJID'])
            logger.error("Runtime error: {0}".format(err))
        except: 
            logger.error('Exception raised in visitcomb RV fit for: ',header.get['FIELD'],apstar.header['OBJID'])
