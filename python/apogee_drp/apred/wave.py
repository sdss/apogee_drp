...
def wavecal(nums=[2420038],name=None,vers='daily',inst='apogee-n',rows=[150],npoly=4,reject=3,
    ...
    if inst == 'apogee-n' :
        ...
        # Post 2021 instrument servicing, lines shifted by 22.5 pixels to the right (~3.167A to the blue)
            ...
            pars0 = [1.19112154e-10,-1.03229705e-05,-2.82422124e-01,16150.139648,
    ...
    #if np.max(linestr['group'])-np.min(linestr['group'])>1:
    ...
    if p['platetype'].strip("'") == 'sky' : 
        ...
        if fps:
            ...
            skyind = np.where(((np.char.array(plugmap['FIBERMAP']['category']).upper() == 'SKY_APOGEE') | 
                              ...
                              (np.array(plugmap['FIBERMAP']['fiberId']) > 0))[0]
        ...
        # SDSS plates
            ...
            if os.path.exists(plugfile)==False:
                if os.path.exists(datadir+'/'+str(p['mjd'])+'/plPlugMapA-'+p['plugmap']+'.par')==True:
