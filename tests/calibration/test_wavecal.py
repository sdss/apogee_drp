...
def test_product_files(tmp_path):
    files = wv.product_files(FakeLoad(tmp_path), 123)
    assert len(files) == 3
...
def patch_common(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(wv, "_make_load", lambda **kwargs: load)
    locks = []
    monkeypatch.setattr(wv.lock, "lock",
...
def write_products(load, number):
    for filename in wv.product_files(load, number):
...
def test_build_wave_workflow(monkeypatch, tmp_path):
    ...
    outputs = wv.build_wave([123, 124], name=123, psfid=50, verbose=True)
    assert outputs == wv.product_files(load, 123)
...
def test_build_wave_nofit_verifies_lines(monkeypatch, tmp_path):
    ...
    def run(frames, **kwargs):
        for frame in frames:
            filename = Path(wv._lines_file(load, frame)); filename.parent.mkdir(parents=True, exist_ok=True)
            filename.write_bytes(b"lines")
    monkeypatch.setattr(wv, "_run_wavecal", run)
    outputs = wv.build_wave([123, 124], psfid=50, nofit=True)
    assert outputs == [wv._lines_file(load, 123), wv._lines_file(load, 124)]


def test_build_wave_existing(monkeypatch, tmp_path):
    load, _, process_calls = patch_common(monkeypatch, tmp_path)
    write_products(load, 123)
    assert wv.build_wave(123) == wv.product_files(load, 123)
...
def test_build_multiwave_uses_available_lines_without_requiring_all(monkeypatch, tmp_path):
    ...
    calls = []
    def run(frames, **kwargs):
        calls.append((frames, kwargs)); write_products(load, 60000)
    monkeypatch.setattr(wv, "_run_wavecal", run)
...
def test_build_multiwave_explicit_dependencies(monkeypatch, tmp_path):
    ...
    def single(frames, **kwargs):
        ...
        for frame in frames:
            path = Path(wv._lines_file(load, frame)); path.parent.mkdir(parents=True, exist_ok=True); path.write_bytes(b"x")
    monkeypatch.setattr(wv, "build_wave", single)
    monkeypatch.setattr(wv, "_run_wavecal",
                        lambda frames, **kwargs: write_products(load, 60000))
...
def test_build_dailywave_forwards_dependency_policy(monkeypatch, tmp_path):
    load, _, _ = patch_common(monkeypatch, tmp_path)
    calls = []
    def run(mjd, **kwargs): calls.append((mjd, kwargs)); write_products(load, mjd)
...
def test_build_dailywave_dependencies_can_be_enabled(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(wv, "_run_dailywave",
                        lambda mjd, **kwargs: (calls.append(kwargs), write_products(load, mjd)))
...
def test_makecal_uses_numbered_wave_builders(monkeypatch, tmp_path):
    calls = []
    monkeypatch.setattr(makecal_v7, "build_wave",
