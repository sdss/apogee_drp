"""Extensive numerical, helper, FITS-I/O, and workflow tests for mkbpm."""
...
from apogee_drp.apred.cal import mkbpm
...
def environment(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(mkbpm.apload, "ApLoad", lambda **kwargs: apload_calls.append(kwargs) or load)
    monkeypatch.setattr(mkbpm.lock, "lock", lambda filename, **kwargs: lock_calls.append((Path(filename), kwargs)))
    monkeypatch.setattr(mkbpm, "PixelBitMask", FakePixelBitMask)
    monkeypatch.setattr(mkbpm.fits, "getdata", getdata)
    monkeypatch.setattr(mkbpm.utils, "software_version", lambda: "git-test")
    monkeypatch.setattr(mkbpm.utils, "reduction_version", lambda: "drp-test")
...
def lockfile(env, bpmid=BPMID):
    return Path(env.load.filename("BPM", num=bpmid, chips=True) + ".lock")
...
class TestMakeBpm:
    def test_empty_masks_produce_zero_mask(self):
        result = mkbpm.make_bpm(np.zeros((5, 7), dtype=np.uint8), pixmask=FakePixelBitMask())
        np.testing.assert_array_equal(result, 0)

    def test_dark_pixels_receive_dark_and_general_bits(self):
        dark = np.zeros((5, 7), dtype=int)
        dark[2, 3] = 8
        result = mkbpm.make_bpm(dark, pixmask=FakePixelBitMask())
    ...
    def test_flat_pixels_receive_flat_and_general_bits(self):
        ...
        flat[1, 4] = 16
        result = mkbpm.make_bpm(dark, flat, pixmask=FakePixelBitMask())
    ...
    def test_badrows_flag_detector_columns_using_historical_orientation(self):
        result = mkbpm.make_bpm(np.zeros((5, 7)), badrows=[1, 5], pixmask=FakePixelBitMask())
        np.testing.assert_array_equal(result[:, 1], BITS["BADPIX"])
        np.testing.assert_array_equal(result[:, 5], BITS["BADPIX"])
        np.testing.assert_array_equal(result[:, [0, 2, 3, 4, 6]], 0)

    def test_badrow_preserves_existing_specific_bits(self):
        dark = np.zeros((5, 7))
        dark[2, 1] = 1
        result = mkbpm.make_bpm(dark, badrows=[1], pixmask=FakePixelBitMask())
        assert result[2, 1] == BITS["BADDARK"] | BITS["BADPIX"]

    def test_empty_badrow_list_changes_nothing(self):
        dark = np.zeros((5, 7))
        dark[2, 1] = 1
        expected = mkbpm.make_bpm(dark, pixmask=FakePixelBitMask())
        actual = mkbpm.make_bpm(dark, badrows=[], pixmask=FakePixelBitMask())
        np.testing.assert_array_equal(actual, expected)

    def test_duplicate_badrows_are_harmless(self):
        result = mkbpm.make_bpm(np.zeros((5, 7)), badrows=[2, 2], pixmask=FakePixelBitMask())
        np.testing.assert_array_equal(result[:, 2], BITS["BADPIX"])

    @pytest.mark.parametrize("badrows", [[-1], [7], [0, 8]])
    def test_rejects_badrows_outside_detector(self, badrows):
        with pytest.raises(ValueError, match="outside the detector"):
            mkbpm.make_bpm(np.zeros((5, 7)), badrows=badrows, pixmask=FakePixelBitMask())

    @pytest.mark.parametrize("shape", [(10,), (2, 3, 4)])
    def test_darkmask_must_be_two_dimensional(self, shape):
        with pytest.raises(ValueError, match="two-dimensional"):
            mkbpm.make_bpm(np.zeros(shape), pixmask=FakePixelBitMask())

    def test_flatmask_shape_must_match(self):
        with pytest.raises(ValueError, match="identical shapes"):
            mkbpm.make_bpm(np.zeros((3, 4)), np.zeros((4, 3)), pixmask=FakePixelBitMask())

    def test_uses_default_pixel_bitmask(self, monkeypatch):
        calls = []
        monkeypatch.setattr(mkbpm, "PixelBitMask", lambda: calls.append(True) or FakePixelBitMask())
        dark = np.zeros((3, 4))
        dark[1, 1] = 1
        result = mkbpm.make_bpm(dark)
...
class TestChipBadrows:
    def test_none_is_preserved(self):
        assert mkbpm._chip_badrows(None, 0) is None
    ...
    def test_simple_objects_are_filtered_by_chip(self):
        ...
        np.testing.assert_array_equal(mkbpm._chip_badrows(entries, 0), [2, 6])

    def test_structured_array_entries_are_supported(self):
        entries = np.array([(0, 3), (2, 8), (2, 9)], dtype=[("chip", int), ("row", int)])
        np.testing.assert_array_equal(mkbpm._chip_badrows(entries, 2), [8, 9])

    def test_mapping_entries_are_supported(self):
        entries = [{"chip": 1, "row": 5}, {"chip": 0, "row": 7}]
        np.testing.assert_array_equal(mkbpm._chip_badrows(entries, 1), [5])

    def test_no_matching_rows_returns_empty_integer_array(self):
        result = mkbpm._chip_badrows([SimpleNamespace(chip=1, row=4)], 0)
...
class TestMkbpmWorkflow:
    def test_requires_darkid_before_constructing_apload(self, monkeypatch):
        monkeypatch.setattr(mkbpm.apload, "ApLoad", lambda **kwargs: pytest.fail("ApLoad should not be called"))
        with pytest.raises(ValueError, match="darkid"):
            mkbpm.mkbpm(BPMID)

    def test_apload_configuration_and_unlock_are_forwarded(self, environment):
        mkbpm.mkbpm(BPMID, apred="test", telescope="lco25m", darkid=DARKID, flatid=FLATID, unlock=True)
    ...
    def test_existing_complete_product_returns_without_reading_inputs(self, environment):
        for filename in output_files(environment):
            filename.touch()
        returned = mkbpm.mkbpm(BPMID, darkid=DARKID, flatid=FLATID)
    ...
    def test_clobber_rebuilds_existing_products(self, environment):
        ...
        mkbpm.mkbpm(BPMID, darkid=DARKID, flatid=FLATID, clobber=True)
    ...
    def test_reads_dark_and_flat_mask_extension_three_for_each_chip(self, environment):
        mkbpm.mkbpm(BPMID, darkid=DARKID, flatid=FLATID)
    ...
    def test_numerical_masks_are_written_as_int16(self, environment):
        ...
        mkbpm.mkbpm(BPMID, darkid=DARKID, flatid=FLATID)
        with mkbpm.fits.open(output_files(environment)[0]) as hdus:
            data = hdus[0].data
    ...
    def test_badrows_are_applied_only_to_selected_chip(self, environment):
        ...
        mkbpm.mkbpm(BPMID, darkid=DARKID, flatid=FLATID, badrow=rows)
        data = []
        for filename in output_files(environment):
            with mkbpm.fits.open(filename) as hdus:
    ...
    def test_primary_header_contains_inputs_versions_and_history(self, environment, monkeypatch):
        monkeypatch.setattr(mkbpm.getpass, "getuser", lambda: "tester")
        monkeypatch.setattr(mkbpm.socket, "gethostname", lambda: "host")
        mkbpm.mkbpm(BPMID, darkid=DARKID, flatid=FLATID)
        with mkbpm.fits.open(output_files(environment)[0]) as hdus:
    ...
    def test_flat_is_optional_and_header_records_no_flat(self, environment):
        returned = mkbpm.mkbpm(BPMID, darkid=DARKID, flatid=None)
        assert returned == [str(filename) for filename in output_files(environment)]
    ...
    def test_lock_is_cleared_after_read_failure(self, environment, monkeypatch):
        monkeypatch.setattr(mkbpm.fits, "getdata", lambda *args, **kwargs: (_ for _ in ()).throw(OSError("boom")))
        with pytest.raises(OSError, match="boom"):
            mkbpm.mkbpm(BPMID, darkid=DARKID, flatid=FLATID)
        assert_lock_cleared(environment)

    def test_lock_is_cleared_after_write_failure(self, environment, monkeypatch):
        monkeypatch.setattr(mkbpm.fits.HDUList, "writeto", lambda *args, **kwargs: (_ for _ in ()).throw(OSError("write failed")))
