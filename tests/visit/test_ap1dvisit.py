...
def test_ap1dvisit_requires_reduction_context_without_backend():
    with pytest.raises(ValueError, match="apred and telescope"):
        ap1dvisit("plan.yaml")


def test_ap1dvisit_accepts_empty_plan_list_without_context():
    assert ap1dvisit([]) == []
