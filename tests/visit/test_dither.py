...
def _shifts(values, sn=None):
    n = len(values)
    signal = np.full(n, 10.0) if sn is None else np.asarray(sn)
    return [ShiftRecord(index=i, framenum=f"{101+i:08d}", shift=value,
                        sn=signal[i]) for i, value in enumerate(values)]
