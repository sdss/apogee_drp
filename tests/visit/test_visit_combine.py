...
from apogee_drp.apred.visit.driver import ShiftRecord
...
def _synthetic_frame(offset, value):
    ...
    for chip in "abc":
        ...
        wcoef[:, 6] = 16000
        wcoef[:, 7] = 100
        chips.append(ChipFrame(np.full(shape, value, dtype=np.float32),
                     np.ones(shape, dtype=np.float32), np.zeros(shape, dtype=np.int16),
                     header={"EXPTIME": 500.0}, filename=f"frame-{value}.fits",
                     ...
                     telluricerr=np.full(shape, 0.01, dtype=np.float32), wcoef=wcoef,
                     lsfcoef=np.array([[2, 0, 0, 0, 1]], dtype=float)))
    return VisitFrame(*chips, shift={"chipfit": np.array([0, offset, offset, offset])})
...
def test_dither_combine_no_dither_sums_frames():
    ...
    shifts = [ShiftRecord(index=0, framenum="1", shift=0, sn=10),
              ShiftRecord(index=1, framenum="2", shift=0, sn=10)]
