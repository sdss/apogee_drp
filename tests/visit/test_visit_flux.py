...
def _frame(npix=240):
    ...
    for letter, limits in zip("abc", wavelength_ranges):
        ...
        chips.append(ChipFrame(flux, np.full_like(flux, 10.0),
                     np.zeros_like(flux, dtype=np.int16), wavelength=wavelength,
                     sky=np.full_like(flux, 100.0), skyerr=np.full_like(flux, 2.0)))
    return VisitFrame(*chips)
