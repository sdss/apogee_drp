...
def _chip(flux, peaks=None):
    chip = ChipFrame(np.asarray(flux, dtype=np.float32),
                     np.ones_like(flux, dtype=np.float32),
                     np.zeros_like(flux, dtype=np.int32))
    chip.peaks = peaks
    return chip
...
def _frame(fluxes, peaks=None):
    ...
    return VisitFrame(*[_chip(flux, chip_peaks) for flux, chip_peaks in zip(fluxes, peaks)])
...
def test_invalid_frame_is_rejected():
    frame1, frame2 = _shifted_spectra()
    frame2.chipc.mask = np.zeros((1, 1), dtype=np.int16)
    try:
